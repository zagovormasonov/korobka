...
- draft_nudge (когда hasDraft=true): пользователь начал заполнять заметку, но не завершил. СТРУКТУРА:
  1. Ценный инсайт (2-3 предложения): если есть draftAnswersSummary — мини-инсайт по частичным ответам (свяжи с диагностикой). Если ответов нет — наблюдение по данным диагностики или целям.
  2. ПОСЛЕДНЕЕ предложение — ОБЯЗАТЕЛЬНО мостик к завершению: пообещай конкретную персональную обратную связь, которую Луми даст ПОСЛЕ завершения заметки. Привяжи обещание к реальным данным/целям. Пример: «Завершите заметку — и я покажу, как ваш уровень X сегодня связан с Y.»
  НЕ упоминай прямо что заметка «не завершена» или «брошена». buttonText не нужен (бэкенд ставит «Продолжить» автоматически).
