...
router.post('/create-credentials', async (req, res) => {
  try {
    ...
    // Автоматически разблокируем персональный план (продукт теперь бесплатный)
    console.log('🔓 [DASHBOARD] Разблокируем персональный план автоматически (бесплатный доступ)');
    try {
      ...
      if (unlockError) {
        ...
        console.log('✅ [DASHBOARD] Персональный план успешно разблокирован');
      }
    } catch (unlockError) {
      console.error('⚠️ [DASHBOARD] Исключение при разблокировке персонального плана:', unlockError);
      // Не прерываем выполнение, продолжаем
    }
