...
Пользователь описал жизненную ситуацию. Ниже в user message передаётся его полный долгосрочный датасет (JSON) — используй его целиком, не сокращай и не придумывай факты, которых нет в тексте ситуации или в датасете.

Сформулируй 5-8 целей, над которыми можно работать именно в ЭТОЙ ситуации.

=== КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА ===

1. ПОНИМАНИЕ СИТУАЦИИ: сначала мысленно выдели из текста ситуации:
   - что произошло (факт, событие)
   - когда по времени (уже случилось / происходит сейчас / в перспективе)
   - что человек чувствует
   - что нельзя изменить (необратимые последствия)
   Если последствия необратимы — цели про проживание, принятие, выводы, а не «предотвратить то, что уже случилось». Не предлагай действия с тем, чего больше нет.

2. ПРИВЯЗКА К ФАКТАМ (без вымысла):
   - Текст ситуации задаёт тему и проблематику целей в первую очередь.
   - Долгосрочный датасет — для уточнения, тона и совместимости с историей пользователя. Не подменяй тему ситуации несвязанными темами из датасета.
   - Не включай в формулировки целей проблемы, симптомы, действия, отношения или жизненные области, которых нет в описании ситуации и которые из него прямо не следуют, если только датасет явно не связывает их с данной ситуацией (например, пользователь в ситуации пишет об обострении уже известного ему паттерна).

3. Каждая цель ДОЛЖНА быть НАПРЯМУЮ связана с описанной ситуацией.
   НЕ генерируй абстрактные цели вроде «улучшить качество жизни» или «развить эмоциональный интеллект», если ситуация конкретная.

4. Цели должны быть конкретными и достижимыми, сформулированы позитивно (к чему идти, а не только «избегать»).

5. САМОПРОВЕРКА для КАЖДОЙ цели:
   a) Это точно про текущую ситуацию пользователя, а не про случайный фрагмент датасета?
   b) Это физически возможно и этично? Не предлагаю ли действие с тем, чего уже нет?
   c) Прочитав цель, пользователь скажет «да, это про мой случай», а не «откуда это взялось?»
   Если хотя бы один ответ «нет» — замени цель.

Верни СТРОГО JSON:
{ "goals": [ { "id": "kebab-case-id", "label": "Текст цели на русском" } ] }`;

    const userMessage = `Ниже сначала текст ситуации (главный источник темы целей), затем полный долгосрочный датасет в JSON без сокращений.

Ситуация пользователя:
${description}

Долгосрочный датасет пользователя (JSON целиком):
${JSON.stringify(dataset ?? {})}`;

    const raw = await callGemini(systemPrompt, userMessage, 0.28);
...
1. НЕ ЗДОРОВАЙСЯ. Не начинай с "Здравствуйте", "Привет", "Рад(а) видеть" и т.д.
   Начинай сразу с сути — наблюдения или инсайта (не с вопроса).
...
4. НИКОГДА НЕ ЗАДАВАЙ ВОПРОСОВ (ни прямых, ни риторических с «?»).
   Экран обратной связи односторонний: пользователь не может ответить Луми.
   Не заканчивай текст вопросом; не используй форматы «как вам…?», «что вы чувствуете?» и т.п.
...
Напиши обратную связь (до 900 символов), используя ОДИН из 4 форматов:
    ...
    const userMessage = JSON.stringify(
      {
        ...
        exerciseTherapyType: exerciseTherapyType || 'DBT',
        goalLabel: goalLabel || '',
        answers: answers || {},
        steps: steps || [],
        dataset: dataset || {},
